...
- ✅ **Visualiseringsfix efter första test:** tog bort `line-dasharray` från Stora vägar eftersom den fick vägar att se hackiga/avbrutna ut. Lagret ackumulerar nu features i klienten per `class:fid` och använder större bbox-padding (80%, max 24 deg²) så vägar inte blinkar av/på när man panorerar mellan fetch-rutor.
...
- ⏭️ **Nästa produktiteration:** Kim vill kunna exponera faktisk hastighet i UI:t, eftersom skillnaden mellan t.ex. 90 och 110 km/h kan vara stor för en rädd förare. `large_roads_in_bbox` returnerar redan `speed_limit` för hastighetsraderna, så nästa steg kan vara tooltip/popup/legend eller ett separat hastighetsläge som gör detta begripligt utan att överbelasta kartan. Tänk igenom hur överlapp med vägtyp-rader ska hanteras: vissa segment kommer från `Vägtyp` och saknar `speed_limit`, medan 90+-raderna har explicit hastighet.

# Current state — 2026-04-26 (popup v2 + self-healing snap + korrekt dedup-strategi + legend + tidsfilter)
...
**Status vid sessionsslut 2026-04-26:** Click-info popup, self-healing snap, korrekt dedup-strategi (0014), legend-widget och tidsfilter är klara. 0013-migrationen är neutraliserad till en `select 1;`-no-op med doc-kommentar — fresh re-deploy hamnar i samma state som 0014 utan att 0013:s fel-premiss körs.

**Föreslagen nästa task:** Punkt 3 nedan, **Om-tjänsten / info-modal** — kort introduktion vid första besöket som förklarar vad kartan visar och datakällorna. Popupen + legenden gör mycket redan; en mini-modal first-time + en "?"-knapp för senare access räcker troligen.
...
5. **Realtidsoverlays** — pågående olyckor (events som inte har `valid_to`) markerade tydligare än historiska.

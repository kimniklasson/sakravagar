...
- **Hjälp** — ruttförslagens filterlogik, kartlager, legender, datakällor, senaste uppdatering samt enkel användnings- och begränsningsinformation.
...
MapLibre-lagren är uppdelade efter frontend cleanup. `web/components/Map/layers.ts` är bara export-yta, medan lagren ligger i `web/components/Map/layers/`: `route.ts` för ruttlinjer/annotationer, `adt.ts` för ÅDT, `largeRoads.ts` för hastighetsbadges, `risk.ts` för vilande risklager, `events.ts` för olycks-/live-lagret, `liveTraffic.ts` för störningar och trafikflöde, `cameras.ts` för globalt kamera-lager och ruttkameror, `bbox.ts` för delad viewport-loader och `popups.ts` för popup-interaktioner. `Map.tsx` är nu en kart-/kontrollkomposition på cirka 390 rader. Routeplanner-orchestration, route state, vald rutt, shared-route loading, planRoute/cache/fetch-flöde, delning, feedback och ruttstopp-sök ligger i `web/components/Map/hooks/useRoutePlannerController.ts` plus de mindre hooks som redan fanns. Info-box och lagerknapp ligger i `InfoBox.tsx` respektive `LayerIconButton.tsx`. Delnings-/feedback-payloads och Google Maps-länkbygge ligger i `web/components/Map/routeSharing.ts`. CSS för ruttplanerare, ruttalternativ, hjälppanel, loading-indikator och ikon-SVG:er är utbruten från `Map.module.css` till komponentnära CSS Modules.
...
- Frontend cleanup är nu på en rimlig nivå efter route/layers-splitten och `useRoutePlannerController`. Fortsätt utan UX-ändring bara när en framtida ändring ändå gör området svårare att läsa.

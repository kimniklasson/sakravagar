...
- **Trafikflöde (snitt)** — ÅDT från NVDB/Lastkajen, blå skala, underlagsdata och inte klickbart. Default på.
...
- `Undvik om möjligt` visas som pills för `Olycksrisk`, `Störningar`, `Höga hastigheter`, `Broar` och `Tunnlar`, direkt mot kartbakgrunden 16 px under inputfälten. Pills kan väljas innan Från/Till fylls i och ligger kvar genom input/geocode. Undvik-score för höga hastigheter räknar fortsatt 90+ km/h-exponering, medan broar/tunnlar räknas från GraphHopper `road_environment` path details.
...
- Ruttalternativ visas som scrollande kort 24 px under pillsen. Varje kort visar titel, kort motivering, tid och distans, har 16 px padding och ett 6 px vänsterstreck med 2 px radius. Vald rutt indikeras med vitt streck; övriga har 30% vitt streck som blir 50% vid hover/focus. Kortens bakgrund är konsekvent `#555` vid 30% opacitet med 16 px blur även för valt alternativ.

...
| `GET /api/cameras` | `bbox` | aktiva Trafikverket-kameror i bbox med direkt bildlänk | 60 s |
...
`/api/cameras` används både av det fristående kamera-lagret och av aktiv-rutt-kameror. Aktiv rutt räknas inte i `/api/route`-svaret; klienten hämtar kameror runt ruttens bbox och filtrerar dem mot vald rutt med 100 meters maxavstånd till polyline.
